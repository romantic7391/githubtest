...
interface UseFilteredTaskHistoryProps {
  page: number;
  pageSize: number;
  filters: SelectHistoriesRequestDto['filters'];
}

export default function useFilteredTaskHistory({ page, pageSize, filters }: UseFilteredTaskHistoryProps) {
  function getInitialData(): SelectHistoriesResponseDto['data'] {
    return {
      histories: [],
  ...
  async function fetchData(): Promise<SelectHistoriesResponseDto['data']> {
    const requestUrl = new URL('/api/history/tasks', window.location.origin);
    if (page) {

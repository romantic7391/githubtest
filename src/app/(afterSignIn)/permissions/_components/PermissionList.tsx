...
export default function PermissionList({ name, page, pageSize }: PermissionListProps) {
  ...
  const { data, isSuccess, fetchStatus, error, isError, refetch } = useFilteredPermissions({
    name,
    page,
    pageSize,
  });
  ...
  function hasActiveFilters(): boolean {
    return !!(name || searchParams.get('searchKey') || searchParams.get('searchValue'));
  }
  ...
  // 빈 데이터 상태
  if (isSuccess && fetchStatus === 'idle' && permissions.length === 0) {
    return (
      <Stack>
        <Card withBorder>
          <Stack align="center" py="xl">
      ...
      <Stack>
        <Grid>
          {permissions.map((permission: Permission) => (
            <Grid.Col key={permission.permissionNo} span={{ base: 12 }}>

...
export async function insertManagerGroup(dto: ManagerGroup) {
  const query = `
...
export async function updateManagerGroup(dto: ManagerGroup) {
  const query = `
...
export async function deleteManagerGroup(no: number, group_no: number) {
  const query = `DELETE FROM \`managerGroup\` WHERE no = ? AND group_no = ?`;
  return exec(query, [no, group_no]);
}
